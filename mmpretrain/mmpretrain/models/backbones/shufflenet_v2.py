...
class ShuffleNetV2(BaseBackbone):
    ...
    def forward(self, x):
        ...
        if lzzsaveflag == 1:
            ...
            import os
            savepath = r"/root/lzz1/Diseases_lzz/weight/mmpretrain/trainsave/shufflenetv2_add4CSASV_LR_pidnet_addmask_addnewloss_v2_changeW*3/ZXIsF3TeDi9BoUv_final"
            initindex = 0
            for nowindex,nowseg in enumerate(x[0]):
                nowindex = nowindex + 1
                if nowindex % 3 == 0:
                    save_image(x[0][initindex:nowindex,:,:], os.path.join(savepath,str(int(nowindex / 3))+".png"))
                    initindex = nowindex
        # lzzappend 
        outs = []
