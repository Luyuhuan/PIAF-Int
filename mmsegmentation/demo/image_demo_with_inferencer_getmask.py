...
def lzzgetfeat_frommmseg(inferencer,inputs):
    nowfeat = {}
    nowshowdir = r"/data2/lyh/Diseases_lzz/weight/mmsegmentation/PIDNet_LR_addnewloss_v2_changeW*3/vis_data"
    lzz_feat = inferencer(inputs, show=False, out_dir=nowshowdir, opacity=0.5)["mask"]
    # lzz_feat = inferencer(inputs, show=False, out_dir=nowshowdir, opacity=0.5)["feature"]
    lzz_feat = [F.interpolate(i.unsqueeze(0).unsqueeze(0).float(),size=[224, 224],mode='bilinear',align_corners=False) for i in lzz_feat]
    # lzz_feat = [F.interpolate(i.unsqueeze(0).unsqueeze(0).float(),size=[336, 336],mode='bilinear',align_corners=False) for i in lzz_feat]
    #  保存特征图像可视化
    # lzzappend
    lzz_saveflag = 0
    if lzz_saveflag == 1:
...
def main():
    ...
    parser.add_argument('--img', default = "/data2/lyh/Diseases_lzz/data/seg_4C_CHD_FGBG/leftImg8bit/val", help='Image file')
    parser.add_argument('--model', default= "/data2/lyh/Diseases_lzz/code/mmsegmentation/configs/pidnet/pidnet-l_2xb6-120k_1024x1024-cityscapes.py",help='Config file')
    parser.add_argument('--checkpoint', default="/data2/lyh/Diseases_lzz/weight/mmsegmentation/PIDNet_LR_addnewloss_v2_changeW*3/iter_17500.pth", help='Checkpoint file')
    parser.add_argument('--out-dir', default="/data2/lyh/Diseases_lzz/weight/mmsegmentation/PIDNet_LR_addnewloss_v2_changeW*3/showout", help='Path to output file')
    parser.add_argument(
        '--show',
        action='store_true',
        default=False,
