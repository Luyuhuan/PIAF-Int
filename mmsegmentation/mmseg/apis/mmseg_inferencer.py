...
class MMSegInferencer(BaseInferencer):
    ...
    def postprocess(self,
        ...
        lzz_feat_flag = 1
        if lzz_feat_flag == 1:
            results_dict['feature'] = [i.seg_logits.data for i in preds]
            return results_dict
        # lzzappend
        ...
        lzz_mask_flag = 0
        if lzz_mask_flag == 1:
            results_dict['mask'] = [i.pred_sem_seg.data[0] for i in preds]
            # lzz 存储mask
            lzzsavemask = 0
            if lzzsavemask == 1 :
                import os
                savepath = r"/data2/lyh/Diseases_lzz/weight/mmsegmentation/pid_4C_CHD/savemask/test/4C"
                picnamelist = [(i.img_path).rsplit("/",1)[1] for i in preds]
                for picname, mask_tensor in zip(picnamelist, results_dict['mask']):
                    mask_np = mask_tensor.cpu().numpy()
                    mask_np = (mask_np * 1).astype(np.uint8)  # 将掩码转换为0和1的值
                    mask_image = Image.fromarray(mask_np)
                    mask_image.save(os.path.join(savepath,picname))
            return results_dict
        # lzzappend
